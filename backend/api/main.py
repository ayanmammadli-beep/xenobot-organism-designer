...
class ProblemInput(BaseModel):
    ...
    fluid_key: Optional[str] = Field(default="water_pure", description="Selected fluid model key")
    contaminant_key: Optional[str] = Field(default="hdpe_microplastic", description="Selected contaminant key")
    flow_speed_cms: Optional[float] = Field(default=1.0, description="Flow speed in cm/s")
    n_contaminants: Optional[int] = Field(default=40, description="Number of contaminant particles")
...
async def create_session(problem: ProblemInput):
    ...
    SESSIONS[session_id] = {
        ...
        "fluid_key":   problem.fluid_key or "water_pure",
        "contaminant_key": problem.contaminant_key or "hdpe_microplastic",
        "flow_speed_cms":  problem.flow_speed_cms if problem.flow_speed_cms is not None else 1.0,
        "n_contaminants":  problem.n_contaminants if problem.n_contaminants is not None else 40,
        "status":      "created",
        "results":     {},
    }
    ...
    try:
        while True:
            ...
            if action == "run_agent":
                ...
                # Run blocking agent in a thread so the event loop stays responsive
                # and we can stream events to the client as they arrive
                ...
                loop.run_in_executor(None, run_agent_in_thread)

                while True:
                    # Wait for next event from thread (blocks a worker thread, not the loop)
            ...
            # ── Run single simulation ───────────────────────
                ...
                await _run_sim_ws(websocket, session, grid, data)

    except WebSocketDisconnect:
        pass


def _env_log_lines(fluid, contaminant, flow_speed_cms: float, n_contaminants: int,
                   sim_steps: int, run_label: str, grid_size) -> list:
    """Build list of log lines for environment and physics parameters (fluid + contaminant)."""
    lines = [
        ...
        "─── Environment ───",
        f"  Run: {run_label}",
        f"  Grid: {list(grid_size)} voxels | sim_steps={sim_steps} | n_contaminants={n_contaminants}",
        f"  Flow speed: {flow_speed_cms} cm/s",
        "",
        "─── Fluid ───",
        f"  Name: {fluid.name}",
        f"  Model: {fluid.model_type}",
        f"  Density: {fluid.density} kg/m³",
        f"  Viscosity: {fluid.viscosity*1e3:.3f} mPa·s",
        f"  Temperature: {fluid.temperature - 273.15:.1f} °C ({fluid.temperature:.2f} K)",
        f"  pH: {fluid.pH}",
        f"  Ionic strength: {fluid.ionic_strength} mol/L",
    ]
    if fluid.model_type != "newtonian":
        lines.append(f"  η₀: {getattr(fluid, 'eta_0', 0)*1e3:.3f} mPa·s | η∞: {getattr(fluid, 'eta_inf', 0)*1e3:.3f} mPa·s")
    if fluid.notes:
        lines.append(f"  Notes: {fluid.notes}")
    lines.extend([
        "",
        "─── Contaminant ───",
        f"  Name: {contaminant.name}",
        f"  Category: {contaminant.category}",
        f"  Interaction: {contaminant.interaction_model}",
        f"  Diameter: {contaminant.diameter*1e6:.1f} µm",
        f"  Density: {contaminant.density} kg/m³",
        f"  Zeta potential: {contaminant.zeta_potential} mV",
        f"  Hamaker: {contaminant.hamaker_constant:.2e} J",
        f"  Binding: {contaminant.binding_mode} | E_bind={contaminant.binding_energy} kT",
        f"  Diffusivity: {contaminant.diffusivity:.2e} m²/s",
    ])
    return lines
...
async def _run_optimization_ws(ws: WebSocket, session: Dict, config: Dict):
    ...
    flow_speed  = session.get("flow_speed_cms", 1.0) * 0.01  # cm/s -> m/s
    flow_cms    = session.get("flow_speed_cms", 1.0)
    n_contam    = session.get("n_contaminants", 40)
    sim_steps   = 150

    # Emit environment and physics params to Metrics log
    lines = _env_log_lines(fluid, contaminant, flow_cms, n_contam, sim_steps, "Run Morpheus", grid_size)
    await ws.send_json({"type": "log_lines", "lines": lines})

    sim_config = SimulationConfig(
        fluid=fluid, contaminant=contaminant,
        flow_velocity=np.array([flow_speed, 0.0, 0.0]),
        grid_size=grid_size, sim_steps=sim_steps, n_contaminants=n_contam,
    )
    ...
    async def send_progress(info):
        data = {k: (v.tolist() if hasattr(v, 'tolist') else v) for k, v in info.items() if k != "grid"}
        await ws.send_json({"type": "progress", "data": data})

    def sync_cb(info):
        """Called from executor thread: schedule progress send on main loop."""
        asyncio.run_coroutine_threadsafe(send_progress(info), loop)

    results = await loop.run_in_executor(
        None,
        lambda: optimizer.optimize(
            evo_gens   = config.get("n_generations", 15),
            bo_iters   = config.get("bo_iterations", 5),
            grad_steps = config.get("grad_steps", 40),
            pop_size   = config.get("pop_size", 12),
            progress_callback = sync_cb,
        )
    )

    # Progress already streamed in real time; no need to replay buffer
...
async def _run_sim_ws(ws: WebSocket, session: Dict, grid: np.ndarray, data: Dict = None):
    from data.contaminants.database import FLUID_DATABASE, CONTAMINANT_DATABASE
    from backend.physics.simulator import SimulationConfig, run_simulation
    import asyncio

    data = data or {}
    fluid_key   = data.get("fluid_key") or session.get("fluid_key", "water_pure")
    contam_key  = data.get("contaminant_key") or session.get("contaminant_key", "hdpe_microplastic")
    flow_cms    = data.get("flow_speed_cms") if data.get("flow_speed_cms") is not None else session.get("flow_speed_cms", 1.0)
    n_contam    = data.get("n_contaminants") if data.get("n_contaminants") is not None else session.get("n_contaminants", 40)
    sim_steps   = 150

    fluid       = FLUID_DATABASE.get(fluid_key, FLUID_DATABASE["water_pure"])
    contaminant = CONTAMINANT_DATABASE.get(contam_key, CONTAMINANT_DATABASE["hdpe_microplastic"])
    try:
        grid_size = tuple(grid.shape) if hasattr(grid, "shape") else (8, 8, 6)
    except Exception:
        grid_size = (8, 8, 6)

    lines = _env_log_lines(fluid, contaminant, flow_cms, n_contam, sim_steps, "Quick simulate", grid_size)
    await ws.send_json({"type": "log_lines", "lines": lines})

    config = SimulationConfig(
        fluid=fluid, contaminant=contaminant,
        flow_velocity=np.array([flow_cms * 0.01, 0.0, 0.0]),
