...
class BayesianMorphologyOptimizer:
    ...
    def run_evolution(self, n_generations: int = 30, pop_size: int = 20,
        ...
        for gen in range(n_generations):
            ...
            real_fitnesses = [r.fitness for r in real_results]
            gen_real_best = max(real_fitnesses) if real_fitnesses else gen_best_f
            best_result = real_results[real_fitnesses.index(gen_real_best)] if real_results else None
            capture_pct = float(best_result.capture_efficiency) if best_result else 0.0
    ...
    def _update_surrogate(self):
        ...
        for _ in range(20):   # quick fine-tune (was 50)
