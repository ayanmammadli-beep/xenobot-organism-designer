...
SYSTEM_PROMPT = """You are the Morpheus AI Scientist — expert in xenobot design and fluid dynamics. Reply like a helpful chatbot: give a clear, short answer first, then use tools only if needed for detail.

IMPORTANT: Start with 1–3 sentences directly answering the user. Then you may use tools (parse_environment, query_xenobot_knowledge, etc.) for deeper analysis. Always end with a brief summary in plain language.

Scientific principles: dimensionless numbers (Re, Pe), Scallop Theorem at low Re, DLVO/vdW/steric interactions, xenobot literature (Kriegman, Levin, Blackiston).

Keep responses concise. Prefer one tool at a time. Always include text that the user can read—never only tool output."""


def _content_block_to_dict(block) -> Dict:
    """Convert API response block to JSON-serializable dict for the next request."""
...
def run_agent(problem_description: str,
    ...
    """
    Run the Claude agent loop with tool use. Streams text as it arrives.
    Yields events: {"type": "text"|"tool_call"|"tool_result"|"done"|"error", "content": ...}
    """
    try:
        client = anthropic.Anthropic(api_key=api_key)
    except Exception as e:
        yield {"type": "error", "content": f"Failed to create API client: {e}"}
        return
    ...
    while True:
        try:
            ...
            err_msg = getattr(e, "message", None) or getattr(e, "content", None) or str(e)
            yield {"type": "error", "content": f"API error: {err_msg}"}
            return
        ...
        if response.stop_reason == "tool_use":
            ...
            for block in response.content:
                ...
                if getattr(block, "type", None) == "tool_use":
                    ...
                    if not isinstance(inp, dict):
                        inp = {}
                    yield {"type": "tool_call", "name": getattr(block, "name", ""), "input": inp}
                    try:
                        result = executor.execute(getattr(block, "name", ""), inp)
                    except Exception as e:
                        result = {"error": str(e)}
                    yield {"type": "tool_result", "name": getattr(block, "name", ""), "result": result}
                    tool_results.append({
                        "type": "tool_result",
            ...
            yield {"type": "done", "content": "Agent completed."}
            break
